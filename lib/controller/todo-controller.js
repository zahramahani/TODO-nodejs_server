...
exports.removeTodo = (request, response, params) => {
  ...
  const acceptRemoveTodo ={
    type:"acceptRemoveTodo",
    Explain:" todo removeed."
   }
...
exports.getTodosByBoardId = (request, response, params) => {
  const valid = ajv.validate(schema.boardIdSchema, params)
  const validatonError = ajv.errors
  const errorTodosByBoardId ={
    type:"errorTodosByBoardId",
  ...
  if (valid) {
    ...
    if (todo) {
      ...
      errorNotFound(response,errorTodosByBoardId )
    }
  } else {
    error(response, validatonError)
  }
