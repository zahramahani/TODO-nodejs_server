...
exports.getNumberofAllTodosTask = (id) => {
  ...
  let query
  if (id) {
    query = pool.query('SELECT COUNT("taskId") FROM "tasks","todos" WHERE"todos"."todoId"="tasks"."todoId"and "tasks"."done"=false  and "boardId"=($1)', [id])
  }
  return query
    .then((res) => {
      result = res
...
exports.removeOwnerDb = (id) => {
  let result

  return pool.query(`DELETE FROM "boards-users" WHERE "boardId"=($1)`, [id])
    .then((res) => {
      result = res
    })
    .then((res) => {
