  ...
  return client
    ...
    .then(() => {
      ...
      // eslint-disable-next-line indent
          [board.owner, board.compelete, board.board_name])
    })
    .then((res) => {
      result = res
      return client.end()
  ...
  return client
    ...
    .then(() => {
      return client.query(`UPDATE "boards" SET "ownerId" =($2),"done"=($3),"name" =($4)  WHERE "boardId"=($1) ;`,
        // eslint-disable-next-line indent
          [id, board.owner, board.compelete, board.board_name])
    })
    .then((res) => {
      result = res
      return client.end()
...
exports.getOwner = (id) => {
  ...
  return client
    ...
    .then(() => {
      let query
      if (id) {
        query = client.query('SELECT "firstName","lastName" FROM "users" and "boards" and "boards-users" WHERE "boards-users" ."boardId" = "boards"."boardId" and "board-user"."userId" = "users"."userId" and "boards"."boardId"=($1),'[id])
      }
      return query
    })
    .then((res) => {
