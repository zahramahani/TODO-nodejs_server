...
exports.getBoards = (id) => {
  ...
  let query
  if (id) {
    query = pool.query('SELECT * FROM "boards" WHERE "boardId"=($1)', [id])
  } else {
    query = pool.query('SELECT * FROM "boards" ORDER BY "boardId" ASC')
  }
  return query
    .then((res) => {
      result = res
...
exports.removeBoard = (id) => {
  ...
  return pool.query('DELETE FROM "boards" WHERE "boards"."boardId"=($1)', [id])
    .then((res) => {
      result = res
    })
    .then((res) => {
...
exports.getUserDb = (boardId) => {
  ...
  let query
  if (boardId) {
    query = pool.query(`SELECT "userName" FROM "users" , "boards" , "boards-users" WHERE "boards-users" ."boardId" = "boards"."boardId" and "boards-users"."userId" = "users"."userId" and "boards"."boardId"=($1) ORDER BY "taskId" ASC`, [boardId])
  }
  return query
    .then((res) => {
      result = res
